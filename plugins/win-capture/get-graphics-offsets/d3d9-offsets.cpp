...
static inline bool d3d9_init(d3d9_info &info)
{
	...
	if (!info.hwnd) {
		printf("failed to create d3d9 get-offset window\n");
		return false;
	}

	info.module = LoadLibraryA("d3d9.dll");
	if (!info.module) {
		printf("failed to load d3d9.dll\n");
		return false;
	}
	...
	create = (d3d9createex_t)GetProcAddress(info.module,
			"Direct3DCreate9Ex");
	if (!create) {
		printf("failed to get 'Direct3DCreate9Ex' offsetess\n");
		return false;
	}

	hr = create(D3D_SDK_VERSION, &info.d3d9ex);
	if (FAILED(hr)) {
		printf("failed to create d3d9ex context: 0x%08lX\n", hr);
		return false;
	}
	...
	if (FAILED(hr)) {
		printf("failed to create d3d9ex device: 0x%08lX\n", hr);
		return false;
	}
