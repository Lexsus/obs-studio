...
static inline bool d3d10_init_format(IDXGISwapChain *swap)
{
...
static bool d3d10_shmem_init(void)
{
...
static bool d3d10_shtex_init(void)
{
...
static void d3d10_init(IDXGISwapChain *swap)
{
	...
	if (success) {
		if (global_hook_info->force_shmem) {
			success = d3d10_shmem_init();
		} else {
			success = d3d10_shtex_init();
		}
	}
...
void d3d10_capture(void *swap_ptr)
{
	...
	if (capture_active()) {
		...
		hr = swap->GetBuffer(0, __uuidof(ID3D10Resource),
				(void**)&backbuffer);
		if (FAILED(hr)) {
