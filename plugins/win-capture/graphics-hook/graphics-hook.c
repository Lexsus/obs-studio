...
static win_pipe_client_t       pipe                            = {0};
...
static inline bool init_pipe(void)
{
	...
	if (!win_pipe_client_open(&pipe, new_name)) {
...
static inline void hlogv(const char *format, va_list args)
{
	...
	if (num) {
		if (!win_pipe_client_write(&pipe, message, num + 1)) {
			win_pipe_client_free(&pipe);
		}
		DbgOut(message);
		DbgOut("\n");
...
inline HMODULE get_system_module(const char *module)
{
	...
	strcat(full_path, "\\");
	strcat(full_path, module);
	return GetModuleHandleA(module);
}

inline HMODULE load_system_library(const char *module)
{
	char full_path[MAX_PATH];
...
inline bool frame_ready(uint64_t interval)
{
...
bool capture_init_shtex(struct shtex_data **data, unsigned int cx,
		unsigned int cy, uint32_t format, bool flip,
		uint32_t handle)
{
...
inline void shmem_copy_data(int idx, void *volatile data)
{
	EnterCriticalSection(&thread_data.data_mutex);
	thread_data.cur_tex = idx;
...
bool capture_init_shmem(struct shmem_data **data, unsigned int cx,
		unsigned int cy, uint32_t pitch, uint32_t format, bool flip)
{
