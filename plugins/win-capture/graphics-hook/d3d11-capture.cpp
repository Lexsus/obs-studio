...
static inline bool d3d11_init_format(IDXGISwapChain *swap)
{
...
static bool d3d11_shmem_init(void)
{
...
static bool d3d11_shtex_init(void)
{
...
static void d3d11_init(IDXGISwapChain *swap)
{
	...
	if (success) {
		if (global_hook_info->force_shmem) {
			success = d3d11_shmem_init();
		} else {
			success = d3d11_shtex_init();
		}
	}
...
void d3d11_capture(void *swap_ptr)
{
	...
	if (capture_active()) {
		...
		hr = swap->GetBuffer(0, __uuidof(ID3D11Resource),
				(void**)&backbuffer);
		if (FAILED(hr)) {
