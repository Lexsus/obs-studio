...
static inline bool init_pipe(struct game_capture *gc)
{
	...
	if (!win_pipe_server_start(&gc->pipe, name, pipe_log, NULL, gc)) {
...
static void game_capture_render(void *data, gs_effect_t *effect)
{
	...
	obs_source_draw(gc->source, obs_get_default_effect(), gc->texture,
			0, 0, gc->global_hook_info->flip, NULL, NULL, NULL);
