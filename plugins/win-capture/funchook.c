...
static inline void hook_reverse_chain(struct func_hook *hook, uint8_t *p)
{
	...
	hook->call_addr = (void*)(hook->func_addr + *((uintptr_t*)&p[1]));
	hook->type      = HOOKTYPE_REVERSE_CHAIN;
	hook->hooked    = true;
...
static inline void hook_forward_chain(struct func_hook *hook, uint8_t *p,
		uintptr_t offset)
{
	uintptr_t cur_offset = *(uintptr_t*)&p[6];
...
static inline void rehook32(struct func_hook *hook, bool force,
		uintptr_t offset)
{
	fix_permissions((void*)(hook->func_addr - JMP_32_SIZE),
			JMP_32_SIZE * 2);
...
#ifdef _WIN64
	hook->is_64bit_jump = (offset > 0x7fff0000);
